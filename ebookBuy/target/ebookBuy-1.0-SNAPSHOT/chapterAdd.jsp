...
<body>
<h1 class="page-title">录入典籍章节</h1>

<div class="form-card">
    <form action="${pageContext.request.contextPath}/chapterAdd" method="post">
        <div class="form-row">
            <label>选择典籍</label>
            <select name="bookId" required>
                <option value="">请选择要录入章节的典籍</option>
                <%-- 这里会遍历所有书籍，不管有多少本都会显示 --%>
        ...
        <div class="form-row">
            <label>章节序号</label>
            <input type="number" name="chapterNum" min="1" value="1" required>
        </div>

        <div class="form-row">
            <label>章节标题</label>
            <input type="text" name="chapterTitle" placeholder="例：第一章 初入仙门" required>
        </div>

        <div class="form-row">
            <label>章节正文内容</label>
            <textarea name="chapterContent" placeholder="请输入章节正文内容，支持换行" required></textarea>
        </div>

        <div class="form-btn-group">
            <button type="submit" class="magic-btn">录入章节</button>
